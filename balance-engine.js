...
class BalanceEngine {
  ...
  processTestFrame(keyPoints, worldLandmarks, timestamp) {
    ...
    // Condition A: Raised foot returns to ground
    ...
    // Condition D: Major corrective movement
    ...
    // Check if any failure persisted long enough
    if (this.failureState.footGround.triggered) {
      this.stopTest('Raised foot touched the ground');
      return { state: 'COMPLETED' };
    }
    if (this.failureState.handHip.triggered) {
      this.stopTest('Hands moved away from hips');
      return { state: 'COMPLETED' };
    }
    if (this.failureState.legContact.triggered) {
      this.stopTest('Legs touched');
      return { state: 'COMPLETED' };
    }
    if (this.failureState.majorCorrection.triggered) {
      this.stopTest('Large corrective movement detected');
      return { state: 'COMPLETED' };
    }
