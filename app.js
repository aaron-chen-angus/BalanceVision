...
class BalanceVisionApp {
  ...
  updateOperatorChecklist(landmarks) {
    ...
    if (!landmarks || landmarks.length === 0) {
      ...
      if (!checks.person) {
        ...
        // Check 3: Hands position — just verify wrists are not raised above shoulders
        // The balance engine handles strict hand-off-hip detection during the actual test
        if (checks.fullbody) {
          checks.hands = true;
        }
  ...
  renderHistoryScreen() {
    ...
    if (results.length === 0) {
      ...
      historyHtml = results.slice().reverse().map(r => `
        <div class="history-item" onclick="app.viewResult('${r.id}')" style="cursor:pointer;">
          <div class="history-item-header">
            <span class="history-id">${r.participantId}</span>
            <span class="history-date">${new Date(r.timestamp).toLocaleDateString()}</span>
          </div>
          <div class="history-item-body">
            <span><strong>${r.duration.toFixed(1)} s</strong> — ${r.supportLeg} support (Trial ${r.trialNumber})</span>
            <span>Score: ${r.stabilityScore}/100 • ${r.swayClassification}</span>
            <span class="history-reason">${r.terminationReason}</span>
          </div>
          <div class="history-item-actions">
            <button class="btn btn-small btn-secondary" onclick="event.stopPropagation(); app.viewResult('${r.id}')">View</button>
            <button class="btn btn-small btn-danger" onclick="event.stopPropagation(); app.deleteResult('${r.id}')">Delete</button>
          </div>
        </div>
      `).join('');
    }
  ...
  viewResult(id) {
    ...
    this.lastTestData = null; // No frame data for historical results
    this.lastCalibrationData = null;
    this.renderScreen(AppState.RESULTS);
  }
  ...
  exportToCSV() {
    ...
    a.href = url;
    a.download = `balancevision_results_${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
