...
class BalanceVisionApp {
        ...
        <div class="operator-guide">
          ...
          <div class="checklist">
            ...
            <div class="checklist-item" id="check-leg">
              ...
              <span class="check-label">${this.participantData.testLeg === 'left' ? 'Left' : 'Right'} leg raised</span>
            </div>
          </div>
  ...
  updateOperatorChecklist(landmarks) {
    ...
    if (!landmarks || landmarks.length === 0) {
      ...
      const avgVis = this.engine.getAverageVisibility(landmarks);
      checks.person = avgVis > 0.3;

      if (!checks.person) {
        hint = 'Person partially detected — ensure good lighting and clear view';
      } else {
        // Check 2: Full body visible (shoulders, hips, knees, ankles, feet)
        const bodyParts = [
          { name: 'shoulders', indices: [LANDMARKS.LEFT_SHOULDER, LANDMARKS.RIGHT_SHOULDER] },
          { name: 'hips', indices: [LANDMARKS.LEFT_HIP, LANDMARKS.RIGHT_HIP] },
          { name: 'knees', indices: [LANDMARKS.LEFT_KNEE, LANDMARKS.RIGHT_KNEE] },
          { name: 'ankles', indices: [LANDMARKS.LEFT_ANKLE, LANDMARKS.RIGHT_ANKLE] },
          { name: 'feet', indices: [LANDMARKS.LEFT_FOOT_INDEX, LANDMARKS.RIGHT_FOOT_INDEX] },
          { name: 'wrists', indices: [LANDMARKS.LEFT_WRIST, LANDMARKS.RIGHT_WRIST] }
        ];

        const missingParts = [];
        for (const part of bodyParts) {
          const visible = part.indices.every(idx => 
            landmarks[idx] && landmarks[idx].visibility >= CONFIG.landmarkConfidenceThreshold
          );
          if (!visible) missingParts.push(part.name);
        }

        checks.fullbody = missingParts.length === 0;

        if (!checks.fullbody) {
          hint = `Cannot see: ${missingParts.join(', ')} — move the participant further from camera`;
        } else {
          // Check if feet are too close to edge of frame
          ...
          if (ankleY > 0.95) {
            ...
            hint = 'Feet are at the edge of frame — move participant further back';
          }
        }
        ...
        if (checks.fullbody) {
          ...
          const torsoLen = keyPoints.torsoLength || 0.3;
          ...
          // Hands are on hips when wrists are close to hips
          checks.hands = leftDist < CONFIG.handHipDistanceThreshold && 
                         rightDist < CONFIG.handHipDistanceThreshold;

          if (!checks.hands) {
            hint = 'Ask participant to place both hands on their hips';
          }
        }
        ...
        if (checks.fullbody) {
          ...
          const expectedLeg = this.participantData.testLeg; // 'left' or 'right' (leg to raise)

          if (detectedLeg === expectedLeg) {
            checks.leg = true;
          } else if (detectedLeg === null) {
            hint = checks.hands 
              ? `Ask participant to raise their ${expectedLeg} foot off the ground`
              : hint;
          } else {
            // Wrong leg raised
            hint = `Wrong leg raised — ask participant to raise their ${expectedLeg} leg`;
          }
        }
      }
    }
    ...
    if (startBtn) {
      ...
      if (allReady) {
        ...
        if (startHint) startHint.textContent = 'Participant is in position — ready to go';
        startHint.className = 'start-hint hint-ready';
      } else {
        startBtn.textContent = 'START TEST';
        if (startHint) startHint.textContent = 'Complete all checklist items to enable start';
