  ...
  updateOperatorChecklist(landmarks) {
    ...
    if (!landmarks || landmarks.length === 0) {
      ...
      if (!checks.person) {
        ...
        // Lenient check: hands just need to be near the torso, not raised or extended out
        if (checks.fullbody) {
          ...
          // Simple check: wrists should be below shoulders and near the body
          // If wrists are between shoulder height and below hips, and not far out sideways, it's fine
          const leftWristBelowShoulder = leftWrist.y > leftShoulder.y;
          const rightWristBelowShoulder = rightWrist.y > rightShoulder.y;
          
          // Check wrists are not extended far from the body horizontally
          const bodyWidth = Math.abs(leftShoulder.x - rightShoulder.x);
          const bodyLeft = Math.min(leftShoulder.x, leftHip.x) - bodyWidth * 0.5;
          const bodyRight = Math.max(rightShoulder.x, rightHip.x) + bodyWidth * 0.5;
          const leftWristInRange = leftWrist.x > bodyLeft && leftWrist.x < bodyRight;
          const rightWristInRange = rightWrist.x > bodyLeft && rightWrist.x < bodyRight;

          checks.hands = leftWristBelowShoulder && rightWristBelowShoulder && 
                         leftWristInRange && rightWristInRange;

          if (!checks.hands && !hint) {
            hint = 'Ask participant to keep hands near their body (on hips)';
          }
        }
